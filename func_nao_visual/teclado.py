...
class TecladoVarredura(tk.Tk):
    def __init__(self):
        ...
        botao_enviar = tk.Button(self, text="Digitar Texto", width=15, height=2, font=('Arial', 12), command=self.enviar_texto)
        ...
        self.canvas = tk.Canvas(self.frame_teclado, highlightthickness=0, bg='SystemButtonFace')
    ...
    def carregar_teclas(self, layout):
        ...
        if layout != self.layout_completo:
            botao_voltar = tk.Button(self.frame_teclado, text="Voltar", width=10, height=2,
                                     command=self.voltar)
        ...
        for r_idx, linha in enumerate(layout):
            for c_idx, tecla in enumerate(linha):
                botao = tk.Button(self.frame_teclado, text=tecla, width=5, height=2,
