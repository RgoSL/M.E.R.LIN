...
class App(CTk):
    ...
    def carregar_preferencias_iniciais(self):
        try:
            ...
            if os.path.exists(config_path):
                ...
                tema_salvo = config.get("tema")

                if idioma_salvo in ["pt", "en", "es", "ar"]:
                    i18n.mudar_idioma(idioma_salvo)
                else:
                    i18n.mudar_idioma(idioma_default)
                ...
                print(f"Preferências carregadas: idioma={idioma_salvo}, tema={tema_salvo}")
                return
            
            i18n.mudar_idioma(idioma_default)
            set_appearance_mode(tema_default)
            print("Preferências padrão aplicadas.")
    ...
    def salvar_preferencia_tema(self, tema):
        try:
            config_path = "config/preferencias.json"
    ...
    def atualizar_tema(self):
        modo = get_appearance_mode()
        nova_cor = "#FFFFFF" if modo == "Light" else "#2B2B2B"
